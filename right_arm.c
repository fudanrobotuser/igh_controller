...
void *rt_thread_function(void *arg)
{
    ...
    while (1)
    {
        ...
        for (i2 = EC_START_P; i2 <= EC_END_P; i2++)
        {
            ...
            else
            {
                ...
                // 判断所有电机都使能
                for (i3 = EC_START_P; i3 <= EC_END_P; i3++)
                {
                ...
                if (isAllEnabled)
                {
                    ...
                    {
                        if (last_position[i2] > defaultPositions[i2] + 800*5 )
                        {
                            target_postion = act_position - 1200*5;
                        ...
                        else if (last_position[i2] < defaultPositions[i2] - 800*5 )
                        {
                            target_postion = act_position + 1200*5;
                            printf("position - %d,  %d to %d  \n", i2, last_position[i2], target_postion);
                            EC_WRITE_S32(domain1_pd + offset[i2].target_position, target_postion);
                    ...
                    // 判断所有电机都零位
                    for (i4 = EC_START_P; i4 <= EC_END_P; i4++)
                    {
...
void Igh_init()
{
    ...
    for (ii = EC_START_P; ii <= EC_END_P; ii++)
    {
...
int main(int argc, char **argv)
{
    ...
    // defaultPositions[0] =  (int)(0.2/(2*PI)*101*pow(2,22));
    // defaultPositions[1] =  (int)(0.2/(2*PI)*101*pow(2,22));
    // defaultPositions[2] =  (int)(0.2/(2*PI)*101*pow(2,22));
    // defaultPositions[3] =  (int)(0.2/(2*PI)*101*pow(2,22));
    // defaultPositions[4] =  (int)(0.2/(2*PI)*101*pow(2,22));
    // defaultPositions[5] =  (int)(0.2/(2*PI)*101*pow(2,22));
    // defaultPositions[6] =  (int)(0.2/(2*PI)*101*pow(2,22));
